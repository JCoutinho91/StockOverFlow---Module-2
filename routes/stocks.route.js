...
router.get("/techcategory", isLoggedIn, (req, res) => {
  const arrayStocks = ["AAPL", "AMZN", "TESL", "MSFT", "UBER", "GOOG", "NVDA", "GME","NFLX","PYPL"];
...
router.get("/healthcategory", isLoggedIn, (req, res) => {
  const arrayStocks = ["BMY", "PFE", "MRK", "MDT", "MRNA", "BSX", "ABBV", "JNJ","GILD","VTRS"];
  const stocksPrs = arrayStocks.map((ticker) => {
    return axios.get(
      `https://www.styvio.com/apiV2/${ticker}/${process.env.API_KEY}`
    );
...
router.get("/industrialcategory", isLoggedIn, (req, res) => {
  const arrayStocks = ["AAL", "CSX", "DAL", "BA", "UAL", "RTX", "LUV", "GE","CARR","HON"];
  const stocksPrs = arrayStocks.map((ticker) => {
    return axios.get(
      `https://www.styvio.com/apiV2/${ticker}/${process.env.API_KEY}`
    );
...
router.get("/financialcategory", isLoggedIn, (req, res) => {
  const arrayStocks = ["BAC", "WFC", "C", "JPM", "MS", "AIG", "HBAN", "KEY","AXP","USB"];
  const stocksPrs = arrayStocks.map((ticker) => {
    return axios.get(
      `https://www.styvio.com/apiV2/${ticker}/${process.env.API_KEY}`
    );
...
router.get("/energycategory", isLoggedIn, (req, res) => {
  const arrayStocks = ["OXY", "MRO", "XOM", "HAL", "APA", "DVN", "CVX", "COP","SLB","KMI"];
  const stocksPrs = arrayStocks.map((ticker) => {
    return axios.get(
      `https://www.styvio.com/apiV2/${ticker}/${process.env.API_KEY}`
    );
...
router.get("/communicationcategory", isLoggedIn, (req, res) => {
  const arrayStocks = ["T", "TWTR", "FB", "ATVI", "VIAC", "DIS", "VZ", "LUMN","EA","TMUS"];
  const stocksPrs = arrayStocks.map((ticker) => {
    return axios.get(
      `https://www.styvio.com/apiV2/${ticker}/${process.env.API_KEY}`
    );
...
router.get("/consumerscategory", isLoggedIn, (req, res) => {
  const arrayStocks = ["KO", "WMT", "KHC", "PG", "KR", "WBA", "MDLZ", "MO","PM","PEP"];
  const stocksPrs = arrayStocks.map((ticker) => {
    return axios.get(
      `https://www.styvio.com/apiV2/${ticker}/${process.env.API_KEY}`
    );
...
router.get("/realestatecategory", isLoggedIn, (req, res) => {
  const arrayStocks = ["HST", "O", "AMT", "WY", "KIM", "PLD", "CBRE", "IRM","VTR","DLR"];
  const stocksPrs = arrayStocks.map((ticker) => {
    return axios.get(
      `https://www.styvio.com/apiV2/${ticker}/${process.env.API_KEY}`
    );
...
router.get("/volumecategory", isLoggedIn, (req, res) => {
  const arrayStocks = ["F", "AAPL", "AMD", "NVDA", "BAC", "MU", "T", "AAL","MSFT","TSLA"];
  const stocksPrs = arrayStocks.map((ticker) => {
    return axios.get(
      `https://www.styvio.com/apiV2/${ticker}/${process.env.API_KEY}`
    );
